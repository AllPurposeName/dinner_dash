...
007 EXAMPLE DATA At least 20 items with varying prices. Some should have multiple categories
...
014 EXAMPLE DATA At least 10 orders with at least 2 at each stage (ordered, completed, cancelled)

## CATEGORIES

015 EXAMPLE DATA At least 5 categories of various sizes
...
## ADMIN DASHBOARD
025 In '/admin' I should see the contents of '/admin/inventory'

################## MILESTONE THREE ##################

## BREEDS
026 In '/breeds' I should see a title of "categories" and a list of breeds with pictures

027 In '/breeds/:id' I should see a Carosel of the breed pictures along with
a description of the breed, a status of the breed (retired?), and a "add random #{breed} cat to cart" button

##CATS

028 In '/cats' I should see a title of "cats" and a list of all the cats

029 In '/cats/:id' I should see a picture of the cat as well as a name, breed,
status, price, and an "add to cart" button

030 In '/admin/cats/:id/edit' I should see a form for changing the cats:
...
031 In '/cart' I should see a table containing columns for:
...
032 In '/users/:id/orders' I should see a titled "orders" and a table for each order.
...
033 In '/admin/orders' I should see a title of "order stats" as well as filter buttons
...
034 In '/admin/orders' I should see a title "order" with a table identical to
...
035 Navigation bar to easily access most of the site. This includes:
...
036 Footer with contact information, link to twitter, etc.

037 Carosel for '/breeds/:id' displaying several pictures per breed.

################## MILESTONE SIX ##################

## AUTHENTICATION

038 Only Admin should be allowed to view the three views rendering '/admin...'

039 Only Admin's nav bar displays Orders/Inventory

040 When a guest visits any unauthroized portion site, they should be redirected to the dashboard.
